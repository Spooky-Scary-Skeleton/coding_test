...
# ----------------------------------
# #[내 풀이] 210327 그나마 기억이 나서 약 46분정도로 풀수 있었다. 
# ----------------------------------
...
while True:
  ...
  #보고있는 방향으로 이동하여 새로운 좌표 확인
  nx = x + dx[d]
  ny = y + dy[d]

  if nx <= 0 or nx >= m or ny <= 0 or ny >=m: #맵의 외곽이거나 벗어날 경우
    continue
  #방문하지 않았고 바다가 아니라면 이동하여 방문처리및 회전횟수 초기화
  if visited[nx][ny] == 0 and area[nx][ny] != 1:
    visited[nx][ny] = 1
    x, y = nx, ny
    turn = 0
    result += 1
...
"""
여전히 시간이 오래걸리긴한다. 조금더 많이 풀어보면서 익숙해질 필요가 있다.
"""
